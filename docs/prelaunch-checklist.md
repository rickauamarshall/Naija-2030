...
- [ ] Free-source enrichment — use Wikidata plus official federation/club/competition sources; API-Football's free tier is optional for recent metadata/statistics. Do **not** automate Transfermarkt access: its current terms prohibit bots/screen scraping and it has no public API. Market values remain manual/unavailable until a licensed source is approved.
...
The remaining identity/profile-level gaps are now resolved: SJK and the NFF confirm Samuel James; the Swiss league and NFF confirm Victory Akpe; and Baník Ostrava plus the NFF confirm Abdullahi Bewene, including his Nigeria debut. Their source links are attached in the site data. Continue to re-check club and call-up details as the pool changes; do not substitute a same-name result or a generic search page.
