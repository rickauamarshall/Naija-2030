...
6. **Methodology/transparency posts.** Occasional posts explaining how the ranking works — form, market value, urgency-to-act — in the same spirit as the site's "Why This Exists" tab. Note: the site no longer publishes an exact ranking-formula breakdown (that composite was deliberately cut from the tab, then called "Methodology," per an editorial call to avoid a false-objectivity vibe — see `prelaunch-checklist.md`), so don't cite specific percentages here as something the site "publishes" unless that decision changes. Boring compared to a goal, but this is what separates an advocacy account from a rumor account — show your work, especially when the account starts getting pushback.
